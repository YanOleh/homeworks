    ...
    try:
        if mode == 'r':
            with open(file_name, 'r') as pb:
                content = pb.read()
                return json.loads(content) if content else {'subscriber': []}
        elif mode in ('w', 'a'):
            with open(file_name, 'w' if mode == 'w' else 'a') as pb:
                json.dump(data, pb, ensure_ascii=False, indent=4)
...
def change_user_data(change, found_contacts, users, file_name):
    if change == 2:
        try:
...
def add_new_user(file_name):
    ...
    data = load_db(file_name, "r")
    data['subscriber'].append(new_user)
    load_db(file_name, 'w', data)
    print('Changes saved!')
